...
工具栏上另有三个开关值得知道：**收起侧栏**（让甘特占满）、**收起筛选**（把筛选区整块收起来给任务让地方，默认展开）、**面板模式**（让面板占满整页，当纯看板用）。
...
- **长期项目**（`long-term: true`）不上甘特图，也豁免所有日期筛选——它只在左侧面板出现，
  卡片右上角的「编辑」按钮是修改它的入口。
- **取消的项目照常出现在甘特图上**。想让图上只看进行中的项目，用筛选栏的状态档
  （默认档「隐藏已完成/取消/归档」会一并隐藏它们）。
- 有项目没上甘特图时，界面顶部会写明**原因和数量**（缺日期 / 长期项目），
  不用自己猜少了哪一条。
...
**无需任何其他插件**：不依赖 Dataview、Templater 等社区插件，甘特图与界面均为插件自绘，装上即用。
