import { HolidayScheduleMap, ProjectMasterSettings, YearHolidaySchedule } from "../types";
import { addDaysIso, daysInMonth, formatIso, isValidIso, parseIso, todayIso } from "../utils/date";

/**
 * 年度法定节假日排期（用户要求 2026-09-20）—— 纯函数，零 DOM。
 *
 * ── 为什么按年维护 ──────────────────────────────────────────────────
 * 国内假期表是逐年公告的，每条都是「几月几号到几月几号」+ 哪几天补班。
 * 让人在导出面板里手打 7 个日期，既啰嗦又容易漏；按年份把**区间**写一次
 * （`10-01~10-07`），导出时按甘特图跨到的年份自动套用。
 *
 ...
 *
 * 这里不接受 `MM-DD`——导出面板里的输入是**跨年度的临时补充**，
 * 没有「当前是哪一年」这个前提可依赖；省年份的写法请写进年度排期表。
 */
export function expandIsoDateSpecs(raw: string): string[] {
	return uniqueSorted(
		expandSpecs(raw, (value, yearOffset) =>
			yearOffset === 0 && isValidIso(value) ? value : null,
		),
	);
}
...
export function expandYearSpecs(raw: string, year: number): string[] {
	return uniqueSorted(expandSpecs(raw, (value, yearOffset) => resolveYearDate(value, year + yearOffset)));
}
...
export function collectHolidayDates(
	...
	const firstYear = parseIso(from).year;
	const lastYear = parseIso(to).year;
...
export function weekendDaysInRange(rangeStart: string, rangeEnd: string): string[] {
	...
	for (let guard = 0; cursor <= to && guard <= MAX_RANGE_DAYS * MAX_YEAR_SPAN; guard += 1) {
		const { year, month, day } = parseIso(cursor);
 ...
 *
 * 抽出来是为了让**自绘甘特图的灰色列**与**导出的 excludes/includes**出自同一份口径。
 * 否则「图上画了什么」和「导出到什么」会各自漂移，正是用户最容易被坑的那种不一致。
 *
 * 注：设置项名字带 `mermaid` 前缀是历史遗留（当初只有导出需要这套日历），
 * 现在自绘甘特图也读它；改名的迁移成本大于收益，这里只做说明。
 */
 ...
 */
export function mergeContiguousDays(days: readonly string[]): { start: string; end: string }[] {
