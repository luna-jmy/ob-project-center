...
/**
 * 可拖动的分栏边界（用户口径 2026-09-21）—— 指针事件实现，与 DragReorder 同一套写法。
 *
 * 只做一件事：把「指针横向移动了多少」换算成目标元素的新宽度，收敛后回调。
 * **落盘不在这里**：拖动过程每帧都写设置等于把磁盘当草稿纸，只在松手（或键盘调整）时
 * 回调一次 onCommit。
 *
 * 宽度由调用方写进 **CSS 变量**，而不是内联 `width`/`flex`：面板模式下侧栏要吃满整页
 * （`.pm-panel-mode .pm-side { flex: 1 1 auto }`），内联样式会把它按死。变量只提供
 * 默认值，样式表里的模式规则照样能覆盖——这是「可拖动」与「面板模式让位」能共存的前提。
 *
 * 上下限由调用方给：上限是**函数**，因为合理的上限取决于容器当下的宽度
 * （窗口变窄后，昨天存的 800px 侧栏会把主区挤没）。
 */
...
/**
 * 宽度收敛（纯函数）：拖动与设置迁移共用同一条规则，避免两处各写一套上下限。
...
export class SplitResizer {
	...
	private finish(): void {
		...
		this.config.handle.ownerDocument.body.removeClass("pm-resizing");
	}
}
