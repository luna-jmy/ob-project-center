...
/**
 * Mermaid 预览的图片导出（用户口径 2026-09-21）—— 把预览 DOM 里的 `<svg>`
 * 变成可落盘的 SVG 文本 / JPEG 二进制。
 *
 * 单独成文件的原因：这里全是 DOM 活（序列化、光栅化），与 MermaidPanel 的
 * 「控件 + 预览」职责无关。仓库对 DOM 层不写测试，所以把能纯化的部分
 * （文件名、附件目录推导、viewBox 解析）单独拆出来——那几条是可测的，
 * 光栅化本身只能在实机上验。
 */
 ...
 * 没有理由让他为导出图片再写一遍路径（用户口径 2026-09-21：参数写过的别再让人重打）。
 *
